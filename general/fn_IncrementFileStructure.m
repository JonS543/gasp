...
%inType = 'file'; inputVal = 'Z:\Jon\proj\toolbox\jontools\test.txt';
%inType = 'dir';  inputVal = 'Z:\Jon\proj\toolbox\jontools\test';

if strcmpi(inType,'file')
    slashLoc = find(inputVal == '/' | inputVal == '\');
    extLoc   = find(inputVal == '.');
    baseDir  = inputVal(1:slashLoc(end)-1);
    baseFile = inputVal(slashLoc(end)+1:extLoc-1);
    baseExt  = inputVal(extLoc:end);
